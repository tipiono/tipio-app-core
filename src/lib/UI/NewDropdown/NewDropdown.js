    ...
    return (
        <div className={cx('dropdown', props.direction)}>
            ...
            <div className={cx('dropdown-menu', props.aligned, { 'show': visible, 'd-none': !visible })}>
                {props.showCloseIcon ? (<a href="#" className="publishedBy__dropdown--close" onClick={(e)=> {
                    e.preventDefault();
...
export function NewDropdownItem(props) {
    return (<a className="dropdown-item" href={props.href ? props.href : "#"} onClick={(e) => { e.preventDefault(); props.hideOnClick && props.setDropDownVisibility(false); props.onClick && props.onClick();}}>{props.children}</a>)
}
